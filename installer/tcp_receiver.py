...
class TCPReceiver:
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            def data_callback(port_index: int, value: float, all_values: List[float]):
                # Store data by device name based on port mapping
                for device_name, ports in self.device_port_mapping.items():
            ...
            self.matlab_receiver.register_device_callback('tcp_receiver', data_callback)
            ...
            for device_name in matlab_configs.keys():
                def make_callback(dev_name):
                    def callback(port_index: int, value: float, all_values: List[float]):
                        if dev_name in self.device_data and port_index < len(self.device_data[dev_name]):
