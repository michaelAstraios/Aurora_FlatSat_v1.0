#!/usr/bin/env python3
"""
ARS Sensor Socket Reader and Rate Sensor Simulator

This application listens to ARS sensor data from 12 sequential ports representing:
...
class SocketReader:
    """Handles socket communication for ARS sensor data"""
    ...
    def _socket_listener(self, sock: socket.socket, port: int, port_index: int):
        """Listen for data on a specific socket"""
        logger.info(f"Starting listener for port {port} (index {port_index})")
    ...
    def start_listening(self) -> bool:
        ...
        logger.info(f"Starting socket readers for {self.ip_address}:{self.start_port}-{self.start_port + self.num_ports - 1}")
        ...
        logger.info(f"Started {len(self.sockets)} socket listeners")
        return True
    ...
    def get_data_summary(self) -> Dict:
        """Get a summary of received data"""
        with self.data_lock:
...
class RateSensorSimulator:
    """Converts ARS data to Rate Sensor format"""
    ...
    def convert_ars_to_rate_sensor(self, ars_data: ARSData) -> RateSensorSimulatedData:
        """Convert ARS data to Rate Sensor format"""
        ...
        self._update_status_words(simulated_data, ars_data)
        
        self.message_counter += 1
        return simulated_data
    
    def _update_status_words(self, simulated_data: RateSensorSimulatedData, ars_data: ARSData):
...
class DataLogger:
    """Logs ARS and simulated Rate Sensor data"""
    ...
    def log_data(self, ars_data: ARSData, simulated_data: RateSensorSimulatedData):
        """Log both ARS and simulated data"""
        log_entry = {
            'timestamp': time.time(),
...
class ARSRateSensorSimulator:
    """Main application class"""
    
    def __init__(self, ip_address: str, start_port: int, log_file: str = None):
        self.socket_reader = SocketReader(ip_address, start_port)
    ...
    def start(self):
        """Start the ARS to Rate Sensor simulation"""
        logger.info("Starting ARS to Rate Sensor Simulator")
        
        # Start socket reading
        if not self.socket_reader.start_listening():
            logger.error("Failed to start socket readers")
            return False
        ...
        logger.info("ARS to Rate Sensor Simulator started successfully")
        return True
    
    def stop(self):
        """Stop the simulation"""
        logger.info("Stopping ARS to Rate Sensor Simulator")
        ...
        logger.info("ARS to Rate Sensor Simulator stopped")
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description='ARS Sensor Socket Reader and Rate Sensor Simulator')
    ...
    simulator = ARSRateSensorSimulator(args.ip, args.start_port, args.log_file)
    
    try:
        if simulator.start():
            print(f"ARS to Rate Sensor Simulator running...")
